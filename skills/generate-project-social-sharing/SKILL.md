...
description: Generate project preview covers for ai-engineering-syllabus projects and complete learn.json social-sharing metadata. Uses project metadata to create .learn/preview.png, update preview URLs, and generate bilingual sharing messages tailored to each project.
...
```bash
python3 course-outline-generator/skills/generate-project-social-sharing/scripts/generate_project_social_assets.py
```
...
- `sharing` is generated by default with bilingual entries (`en`, `es`)
...
The script writes `sharing` as an array of bilingual message entries:
...
- Verify `sharing` is present and contains bilingual (`en`/`es`) messages
